	...
	unsigned int wr_size, unsigned int rd_size)
{
	...
	rd_size = (tcm_dev->platform_rd_size == 0) ?
		rd_size : MIN(rd_size, tcm_dev->platform_rd_size);
...
static int syna_tcm_v1_check_max_rw_size(struct tcm_dev *tcm_dev)
{
	...
	return syna_tcm_v1_set_up_max_rw_size(tcm_dev, max_write_size, tcm_dev->max_rd_size);
}
	...
	unsigned char *data, unsigned int size, unsigned int data_len)
{
	...
	syna_tcm_v1_check_max_rw_size(tcm_dev);
...
/*
 *  Assemble the TouchComm v1 packet and send the packet to device.
 *
 * param
 *    [ in] tcm_dev:     pointer to TouchComm device
